 ...
 * para el APK, http://localhost:5173/5174 para dev). Sin definir → '*', para
 * que el entorno local funcione sin configurar nada.
 */
...
/** Cabeceras CORS de la petición: eco del Origin solo si está en la lista. */
export function corsDe(req: Request): Record<string, string> {
  if (!ORIGENES.length) return COMODIN
