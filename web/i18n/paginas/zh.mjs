...
export const TEXTOS = {
  ...
  'og.desc':
    '把你的生活装进 3D 房子：习惯、目标、财务、饮食等等。一次支付 8.99 美元，首月含 AI；免费试用。',
  ...
  'car.studio.t': '一个用来创作的工作室',
  'car.studio.p':
    '单独的四个应用：音频、艺术、写作和视频。用钢琴卷帘和 MIDI 键盘作曲，绘画并修图，一章一章写书，按剧本剪视频——做好的都能导出，也可以把视频直接发布到你自己的 YouTube、TikTok、Facebook 或 Instagram 账号。',
  'car.2.t': 'AI 助手',
  'car.2.p':
    '和助手聊天：记录饮食、创建例行、规划目标、生成图片和 3D 模型。首月自带 700 点额度；之后 AI 就是可选的了。',
  'car.3.t': '全面同步',
  'car.3.p': 'MindHaOS 会跟你到手机、平板和电脑。传输全程加密，并在云端备份。开通 Pro 后，你的云端（“文件”房间）还能保存 10、30 或 100 GB 的文件。',
  'car.4.t': '玩起来像一款游戏',
  'car.4.p':
  ...
  'faq.3.a':
    '你说了算，也可以一分不花。订阅每月 6 美元（700 点额度和同步），或每年 60 美元——白送两个月。如果不够用，×2 和 ×3 档每月 12 或 18 美元，给 1400 或 2100 点。如果不想订阅，还有单独充值：6 美元 700 点，不会过期，只有你提出请求时才消耗。这部分确实在<a href="/cuenta">你的账户</a>里支付，对你所有设备都有效。每个等级还包含云端空间：10、30 或 100 GB。',
  'faq.4.q': 'AI 额度是什么？',
  'faq.4.a':
    '向助手发出的每个请求，按处理成本计费的单位：一次普通回复算 1 点，一份长计划 4 点，一张图 3 点（高质量 10 点），一个 3D 模型 10 点。从不自动扣费：只有你提出请求时才会消耗。',
  'faq.5.q': '如果我取消会怎样？',
  'faq.5.a':
    '整个应用和设备上所有数据都留给你，以本地模式运行。你失去的只有每月额度和同步。重新续订后，一切都会按你离开时的样子恢复。云端文件会保留 90 天只读，方便你下载；之后会被删除。',
  'faq.6.q': '我的数据存在哪里？',
  'faq.6.a':
    '先存在你的设备上（App 是本地优先），开启同步后也会存到云端，方便在设备之间切换。本地模式下不会有任何数据离开你的设备。付款由 RevenueCat 和 Stripe 处理——如果你从手机购买，则由应用商店处理：我们从不接触你的银行卡。详见<a href="/privacidad">隐私政策</a>。大文件（你的云端和 Studio 媒体）存储在 Cloudflare R2。',
  'faq.7.q': '哪些设备能用？',
  'faq.7.a':
  ...
  'priv.datos.7':
    '<strong>位置：</strong>只在你使用时获取。为了计算路线以及在导航过程中，你的精确位置和目的地会发送给 HERE Technologies；带 GPS 的有氧运动会把训练路线和你的数据一起保存。我们绝不会把它分享给其他用户。',
  'priv.datos.8':
    '<strong>消息和共享内容：</strong>如果你选了一个昵称并添加了联系人，我们会保存你的昵称、显示名称、表情符号和你的角色头像，以及你发送的消息、文件和共享空间，以便把它们送达你选择的人。只有参与那段对话或那个空间的人能看到。如果你举报了什么，我们会保留一份被举报内容的副本以便审核。',
  'priv.datos.9':
    '<strong>浏览器：</strong>浏览历史和每个网站的停留时间都保存在你的设备上。只有当你开启同步时，它们才会上传到我们的服务器；只有当你请求分类时，域名才会发送给 AI。',
  ...
  'priv.borrar.p':
    '在应用里：编辑器 → 设置 → 账户。删除会清除你的用户、同步数据和文件；只保留法律要求留存的账务记录。从未完成任何购买的账户会在创建 3 天后自动删除，不保留其任何数据。',
  ...
  'term.precio.2':
    '每档每月含 700 点 AI 额度（700 / 1400 / 2100）以及设备间同步。当月未用完的额度不累积到下个月。还包含云端空间：按等级为 10、30 或 100 GB（年付和赠送的一个月为 10 GB）。',
  'term.precio.3':
  ...
  'term.cancelacion.p':
    '你可以随时在<a href="/cuenta">你的账户</a>里通过「管理订阅」取消；已付费的周期结束前，方案照常有效。之后应用会继续在你的设备上以本地模式运行，只是没有每月额度和同步。剩下的充值额度仍然可用，你也可以随时续订。你在云端的文件会保留 90 天只读，方便你下载；超过期限后将从我们的服务器删除。',
  'term.datos.h': '你的数据',
  'term.datos.p':
  ...
  'term.comunidad.h':
    '社区规范',
  'term.comunidad.p':
    'MindHaOS 让你给联系人写消息、分享房间里的内容、访问其他人的 MindHaOS，以及在共享空间里协作。使用这些功能即表示你接受以下规范：',
  'term.comunidad.1':
    '不要发送骚扰、威胁、仇恨或歧视内容、色情内容、暴力内容、垃圾信息、诈骗或任何违法内容。',
  'term.comunidad.2':
    '我们不容忍滥用性内容。在应用里，你可以举报任何消息、联系人或空间成员，也可以拉黑任何人。',
  'term.comunidad.3':
    '我们会在 24 小时内处理每一条举报：删除违反这些规范的内容，并把发送者移出服务。',
  'term.comunidad.4':
    '你对自己发送和分享的内容负责。你也可以通过邮件告诉我们（见“联系方式”）。',
  'term.cambios.h': '服务变更',
  'term.cambios.p':
