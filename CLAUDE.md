...
| J | Archivo (id `archivos`) | La nube Pro tipo Drive: carpetas y archivos de cualquier tipo con vista previa (imagen, video, audio, PDF, texto), 10/30/100 GB según el nivel. Los bytes viven en Cloudflare R2 (Edge Function `almacen`, `core/cuenta/almacen.ts`); las filas `archivosNube`/`carpetasArchivo` solo llevan metadatos y viajan por el sync. Ver `docs/BACKEND.md` §4 | ✅ |
