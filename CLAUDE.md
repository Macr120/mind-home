...
- **App Review de Apple (los seis envíos, el error 8 de StoreKit, Sign in with Apple nativo, cómo reenviar)**:
...
| J | Archivo (id `archivos`) | La nube Pro tipo Drive: carpetas y archivos de cualquier tipo con vista previa (imagen, video, audio, PDF, texto), 10/30/100 GB según el nivel. Los bytes viven en Cloudflare R2 (Edge Function `almacen`, `core/cuenta/almacen.ts`); las filas `archivosNube`/`carpetasArchivo` solo llevan metadatos y viajan por el sync. Con la forma de Google Drive: menú lateral (Mi Archivo, **Cuartos**, Recientes, Destacados, Papelera), cuadrícula o lista, selección múltiple, clic derecho, buscador y orden; arrastrar y soltar con el gesto de la casa (`core/ui/comun/arrastre.tsx`, vale en táctil) y carpetas enteras desde el sistema. Cada cuarto de la casa tiene su carpeta: arriba lo que su app ya guarda (`rooms/archivos/fuentes.ts`, solo lectura) y debajo tus archivos. Los medios del Studio suben SOLOS a la nube (`core/studio/nubeStudio.ts`) y se bajan al usarlos en otro dispositivo. Papelera de 30 días (`core/cuenta/papelera.ts`) y enlaces públicos `mindhaos.com/d/<token>` (función `archivo-publico`). Sin plan: 90 días de solo lectura y luego `almacen-purga`. Ver `docs/BACKEND.md` §4 | ✅ |
