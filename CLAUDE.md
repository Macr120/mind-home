...
| J | Archivo (id `archivos`) | La nube Pro tipo Drive: carpetas y archivos de cualquier tipo con vista previa (imagen, video, audio, PDF, texto), 10/30/100 GB según el nivel. Los bytes viven en Cloudflare R2 (Edge Function `almacen`, `core/cuenta/almacen.ts`); las filas `archivosNube`/`carpetasArchivo` solo llevan metadatos y viajan por el sync. Pestaña «Del Studio»: los medios del Studio suben SOLOS a la nube (`core/studio/nubeStudio.ts`) y se bajan al usarlos en otro dispositivo. Sin plan: 90 días de solo lectura y luego `almacen-purga`. Ver `docs/BACKEND.md` §4 | ✅ |
