...
/**
 * Reordenar a mano las filas de la agenda arrastrándolas.
 *
 * Se agarra la FILA ENTERA, sin asa: mantenerla pulsada un momento (o empezar a
 * moverla con el ratón) la levanta. Por eso el gesto no captura el puntero de
 * entrada —eso dejaría muertos los botones de dentro y el scroll de la lista—,
 * sino que espera a saber qué es: un toque corto sigue siendo un toque, moverse
 * enseguida con el dedo es hacer scroll, y solo la pulsación larga arrastra.
 *
 * La fila solo se mueve DENTRO de su propia lista: una tarjeta de pendientes no
 * puede acabar entre los medicamentos, ni salir de la carpeta en la que está.
 * Por eso cada fila declara a qué lista pertenece y el destino se descarta en
 * cuanto no coinciden; el tablero Kanban, que sí cambia de columna, usa el mismo
 * gesto con otro destino (ver `Tablero.tsx`).
 *
 * No se usa el drag & drop de HTML5 (`draggable`/`onDragOver`): no existe en
 * táctil y la casa se usa en Android. `document.elementFromPoint` dice sobre qué
 * fila está el dedo, como en `HojaPlan` y `rooms/computo/useArrastreArbol`.
 */
...
/** Fila que guarda su puesto manual: todas las tablas de la agenda lo tienen. */
...
export function useArrastre<D>(
  ...
  // Si la app se cierra a mitad de un arrastre, el freno del scroll se quedaría
  // puesto en todo el documento.
  useEffect(() => () => {
    ...
    fila: (item: T) => {
      ...
      return {
        ...
        className: `cursor-grab rounded-xl ${enMano === k ? 'opacity-40' : ''} ${
          cae ? (cae.debajo ? 'border-b-2 border-emerald-400' : 'border-t-2 border-emerald-400') : ''
        }`,
      }
    },
  }
