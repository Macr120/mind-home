...
export function MetasTab({
  ...
  // La meta de peso vive aquí y no dentro de la TDEE: sin ella `progresoMeta()`
  // devuelve null y toda la pantalla de Progreso se queda sin nada que enseñar.
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-xl bg-white/5 p-4 border border-white/10 space-y-3">
        <p className="text-sm font-semibold"><Icono nombre="balanza" /> {t('cocina.metas.metaPeso', 'Tu meta de peso')}</p>
        ...
        >
          {t('cocina.metas.guardarMeta', 'Guardar meta')}
        </button>
      </div>
      ...
      <div className="rounded-xl bg-white/5 p-4 border border-white/10">
        ...
        <CalculadoraTdee perfil={p} objetivo={objetivo} onAplicar={(k) => setCalorias(String(k))} />
      </div>
    </div>
  )
}
