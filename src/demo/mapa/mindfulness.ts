 ...
 *
 * El jardín es un cuarto SIN MUROS con la app de mindfulness (UNA celda: el
 * resto del rincón se queda de jardín abierto), y la alberca es un cuarto de
 * sótano de 3×2 (nivel −1): el agua nadable solo existe ahí. Ambos van
 * redondeados con recortes finos — no llevan ventanas, así que la curva no le
 * quita nada a la fachada.
 *
...
export async function construirMindfulness(cols: number, rows: number): Promise<string> {
  ...
  /** Redondea las 4 esquinas exteriores de un cuarto de `anchoxlargo` celdas.
   *  En un 1×1 son los cuatro cuadrantes de su única celda. */
  const redondear = async (id: string, ancho: number, largo: number) => {
    const c = ancho - 1
    const r = largo - 1
    await L().pintarSubformaCelda(id, 0, 0, 0, 'circular') // NO
    await L().pintarSubformaCelda(id, c, 0, 1, 'circular') // NE
    await L().pintarSubformaCelda(id, 0, r, 2, 'circular') // SO
    await L().pintarSubformaCelda(id, c, r, 3, 'circular') // SE
  }

  // ── Jardín zen: UNA celda, sin muros (lo marca su plantilla al asignarse);
  // lo que era el resto del cuarto queda de jardín abierto. OJO al offset +1
  // de la fila: ver la nota del spawn en la cabecera. ──────────────────────
  const jc = u.c0 + 2
  const jr = u.r0 + 1
  ...
  await D().addObjeto(MAPA_ROOM, 'recurso:106', '#7c4a24', undefined, mundo(u.c0 + 2, u.r0 + 2.4))
