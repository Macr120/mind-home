...
export function WallEditor({ roomId }: { roomId: string }) {
  ...
  const [modo, setModo] = useState<Modo>('pared')
  const [sel, setSel] = useState<{ off: Cell; side: SideKey } | null>(null)
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-1 rounded-lg bg-black/20 p-1">
        {MODOS.map((m) => (
          <button
            key={m.id}
            type="button"
            onClick={() => { setModo(m.id); setSel(null) }}
            className={`flex-1 rounded-md py-1.5 text-[11px] font-semibold transition ${
              modo === m.id ? 'bg-white/15 text-white' : 'text-white/45 hover:text-white/70'
            }`}
          >
            {labelModo(m.id)}
          </button>
      ...
      {/* 2 · Croquis (seleccionar el muro a editar) */}
      <div
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow: seleccionado ? '0 0 0 2px rgba(255,255,255,0.9)' : undefined,
      ...
      {sel && selEdge && (
        <div className="space-y-3 rounded-xl border border-white/10 bg-white/5 p-2.5">
          <p className="text-[11px] font-semibold text-white/70">
            ...
            <span className="text-white/90">{labelModo(modo)}</span>
          </p>
          ...
          {modo === 'ventana' && (
            <div className="space-y-2">
              ...
              <div className="flex gap-1.5">
                ...
                >
                  🌈 {t('paredes.multicolor', 'Multicolor')}
                </button>
              </div>
              {!vVentMulticolor && (
                <ColorPicker
              ...
              {muroImagen ? (
                <div className="space-y-2">
                  <div
                    ...
                    style={{ height: 64 }}
                  >
                    <img src={muroImagen} alt="Muro" className="h-full w-full object-cover" />
                    {imagenActiva && (
