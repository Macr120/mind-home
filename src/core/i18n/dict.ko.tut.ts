...
export const KO_TUT: Dict = {
  ...
  'tut.casa.4.texto':
    '방금 눈앞에서 보여 드렸어요. 아이소메트릭, 3인칭, 1인칭이죠(V 키로도 바뀌어요). \'아이소\'를 누르면 카메라가 캐릭터 위로 다시 맞춰져서, 멀리 돌아다니다 왔을 때 빠르게 돌아오는 길이 돼요.',
  ...
  'tut.metas.2.titulo': '같은 시트를, 양쪽에서',
  'tut.metas.2.texto':
    '목표를 누르면 시트가 열리고, 계획이 있으면 계획도 열려요. 그 앱에서 눌렀을 때 나오는 것과 같은 플래너인데, 차이가 하나 있어요. 거기서는 그 앱으로 좁혀지고, 여기서는 집 전체의 목표까지 통째로 보여요.',
  'tut.metas.3.titulo': '달성은 그 앱에서, 여기가 아니라',
  'tut.metas.3.texto':
    '목표가 오늘 요구하는 일은 이 방에서 하지 않아요. 그 목표를 맡은 앱의 미션에, 그날의 다른 일들과 섞여 나와요. 거기서 실제로 기록해야 달성돼요. 여기서는 계획하고, 앱이 실행해요.',
  'tut.metas.4.titulo': '날짜는 캘린더로',
  'tut.metas.4.texto': '기한이 있는 목표는 다른 일정처럼 시계의 캘린더에 나오고, 계획의 하위 목표도 함께 나와요. 한쪽에서 계획하면 다른 쪽의 한 해가 저절로 채워져요.',
  'tut.metas.5.titulo': '그래서 중심에 있어요',
  'tut.metas.5.texto':
    '앱마다 자기 몫을 챙기지만, 세운 목표를 한자리에서 볼 수 있는 건 이 방뿐이에요 — 그리고 여기서 캘린더로, 각 방의 미션으로 퍼져 나가요. 모든 게 교차하는 자리가 되는 것, 그게 이 방의 일이에요.',
