...
export const KO_TUT: Dict = {
  ...
  'tut.hoy.8.texto':
    '방 위에 뜬 빨간 표시는 오늘 그곳에 남은 미션 수예요. 같은 숫자가 홈 화면에도, 들어가기 말풍선에도, 방의 가구 위에 떠 있는 오브에도 나와요. 시간이 지난 게 있으면 호박색으로 바뀌고요. 표시가 없으면 그 방은 오늘 할 일을 다 마친 거예요.',
  'tut.hoy.9.titulo': '그리고 캘린더에 전부 모여서',
  'tut.hoy.9.texto':
    '시계의 미션 버튼은 집 전체에서 오늘 할 일을 앱별 카드로 모아 줘요. 왼쪽은 남은 것, 오른쪽은 이미 끝낸 것이에요. 여기서는 기록하지 않아요. 각 줄을 누르면 해당 앱으로 가고, 데이터는 거기에 적혀요.',
